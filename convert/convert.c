...
int convert565(char *in, char *out, int vid_txt, int mode );
void usage(char *argv[]);

int mode = 0; //The input mode; 565 is png or jpg input, converted to 65536 colors; 256 is gif input, converted to 256 colors (color palette); 2 is black/white
...
int main(int argc, char *argv[]){
	for (int i=1; i<argc; i++){
		...
		else if (strcmp("-b",argv[i])==0) mode = 2;
	...
	if(mode==565||mode==2){
		...
		while (current<=last){
			...
			if(mode==565){
				...
				sprintf(outfile,"%s/%04d.bin",arg_out,outNr++);
			}
			if (verbose)
				printf("convert565(\"%s\", \"%s\");\n",infile, outfile);
			convert565(infile, outfile, current==first, mode);
			current+=step;
		}
	}//if mode==565||mode==2
	else{//mode==256
		...
		if(verbose){
		    ...
		    int i=0; while (i<255){
		            printf("\
		2;%d;%d;%dm  \033[48;2;%d;%d;%dm  \033[48;2;%d;%d;%dm  \033[48;2;%d;%d;%dm  \
		2;%d;%d;%dm  \033[48;2;%d;%d;%dm  \033[48;2;%d;%d;%dm  \033[48;2;%d;%d;%dm  ",
...
args:\n\
  ...
  -g         256 colors, input file is a single .gif animation\n\
  -p         full color, input file is a sequence of .png or .jpg files (default)\n\
             use %%04s in the filename as a replacement for the 4 digit number.\n\
	     (or %%03s for 3 digits)\n\
  -b         black/white (uses png or jpg files like -p)\n\
\n\
Example:\n\
...
int convert565(char *in, char *out ,int vid_txt, int mode){
	...
	unsigned char *img;
	if(mode==565){
		img = stbi_load(in, &w, &h, &c, 3);//load with 3 channels (rgb)
	}else{//mode==2
		img = stbi_load(in, &w, &h, &c, 1);//load with 1 channel (monochrome)
	}
	if (img == NULL){
		fprintf(stderr,"Can't find \"%s\".\n",in);
		return -1;
	...
	if(mode==565){
		...
		unsigned char color_pixel = (img[i++]>=0xc0)?0x80:0x00; // read the first pixel
		int converting = 1;
		while (converting){
			unsigned char color_block = color_pixel;
			//printf("color of block: %d ",color_block);
			unsigned int  amount = 0;
			while(1){
				amount++;//add this pixel to the current block
				if(i==w*h){converting=0;break;}//finish block if end of the image
				color_pixel = (img[i++]>=0xc0)?0x80:0x00;//read the next pixel
				if(color_pixel!=color_block) break;//finish block if different color
				if(amount==128) break;//finish block if full
			}
			//printf("no of pixels: %d\n", amount);
			//the block is finished, either because of a color change or because the amount reached the max value, so output it to the file.
			amount&=0x7f; //clear the top bit (so 128 becomes 0)
			fputc(color_block|amount, fp);
			//the top bit contains the color, the lowet 7 bis contain the amount of pixels in this color.
		}
	}
	fclose(fp);
	return 0;
